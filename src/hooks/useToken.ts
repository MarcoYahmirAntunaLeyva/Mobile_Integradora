...
const useTokenManagement = ({ setIsCheckingToken }: TokenManagementProps) => {
  ...
  const renewToken = async (token: string) => {
    try {
      const response = await fetch(`https://hostbackend-production-06a9.up.railway.app/auth/timeTokenLife`, {
        method: "PUT",
        headers: {
  ...
  const validateToken = async () => {
    try {
      ...
      const response = await fetch(`https://hostbackend-production-06a9.up.railway.app/api/auth/timeTokenLife`, {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
